...
const PostCard: React.FC<{ 
  ...
  const handlePreview = () => {
    ...
    previewHtml = previewHtml.replace('<img src="images/hero.png"', `<img src="${post.images[0]}"`);
    previewHtml = previewHtml.replace('<img src="images/features.png"', `<img src="${post.images[1]}"`);
    previewHtml = previewHtml.replace('<img src="images/cta_banner.png"', `<img src="${post.images[2]}"`);
    ...
    <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-4 flex-grow flex flex-col">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <button onClick={handleDownloadZip} className="text-sm bg-red-600 hover:bg-red-700 text-white py-2 px-3 rounded-md flex items-center justify-center gap-2">
            <ArrowDownTrayIcon className="h-4 w-4" /> Download ZIP
          </button>
          <button 
            onClick={() => onPublish(post)}
            className="col-span-2 text-sm bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 px-3 rounded-md flex items-center justify-center gap-2 font-semibold shadow-lg transition-all transform hover:scale-105"
          >
            <GlobeAltIcon className="h-5 w-5" /> Publish Live
          </button>
        </div>
        <button onClick={() => onDelete(post.id)} className="mt-3 w-full text-sm bg-gray-900 hover:bg-opacity-50 text-red-400 py-2 px-3 rounded-md flex items-center justify-center gap-2">
...
const MyPostsPage: React.FC = () => {
  ...
  const handlePublish = async (post: GeneratedPost) => {
    const slug = (post.productName || post.templateName || 'post')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '')
      .substring(0, 50);

    if (!confirm(`آیا این پست منتشر بشه؟\n\nلینک بعد از deploy:\nhttps://${slug}.netlify.app`)) return;
    ...
    try {
      const res = await fetch('/api/publish', {
        ...
        body: JSON.stringify({
          ...
          images: post.images,
          title: post.productName || post.templateName,
        }),
      });
      ...
      if (data.success) {
        alert(`موفقیت!\n\nلینک زنده:\n${data.url}\n\n(بعد از deploy روی Netlify فعال میشه)\n\nZIP هم دانلود شد!`);
        
        const a = document.createElement('a');
        a.href = data.zipDownload;
        a.download = `${slug}.zip`;
        a.click();

        setTimeout(() => window.open(data.url, '_blank'), 1500);
      } else {
        alert('خطا در انتشار: ' + data.error);
      }
    } catch (err) {
      alert('خطای اتصال — مطمئن شو /api/publish/route.ts درست باشه');
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      {posts.length === 0 ? (
        <p className="text-gray-400 text-center text-lg">هنوز پستی نساختی. برو به صفحه Create Post!</p>
      ) : (
