...
const CreatePage: React.FC = () => {
  ...
  const [generationProgress, setGenerationProgress] = useState(0); // 0: idle, 1-4: loading steps
  ...
  const handleDownloadZip = async (dataToZip: GenerationResult) => {
    const zip = new JSZip();
    const htmlContent = atob_utf8(dataToZip.previewHtmlBase64);
    zip.file("index.html", htmlContent);
    const imgFolder = zip.folder("images");
    if (imgFolder) {
      dataToZip.images.forEach((imgData, index) => {
    ...
    saveAs(blob, `${productName.replace(/ /g, '_')}_${dataToZip.templateName.replace(/ /g, '_')}.zip`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await generateAffiliatePost(apiKey!, productName, productDescription, affiliateHoplink, templateStyle, handleProgress);
      const newPost = {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {generatedData && (
        <div className="bg-green-800 border border-green-600 text-white p-4 rounded-lg mb-6 flex items-center justify-between">
            <p className="font-semibold">Success! Your post has been generated and saved.</p>
            <div className="flex gap-2">
                <button onClick={() => handleDownloadZip(generatedData)} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md text-sm">Download ZIP</button>
                <button onClick={handleNewVariation} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-md text-sm">Save & New Variation</button>
            </div>
        </div>
      )}
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="affiliateHoplink" className="block text-sm font-medium text-gray-300">Affiliate Hoplink</label>
